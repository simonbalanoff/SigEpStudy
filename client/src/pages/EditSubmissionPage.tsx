...
export function EditSubmissionPage() {
    ...
    const [resourceType, setResourceType] =
        useState<ResourceType>("study_guide");
    ...
    const mutation = useMutation({
        mutationFn: (input: Record<string, unknown>) =>
            request<{ resource: Resource }>(`/resources/${resourceId}`, {
                method: "PATCH",
    ...
    const submit = async (event: React.FormEvent) => {
        ...
        try {
            ...
            toast.success(
                "Submission updated and returned to the review queue.",
            );
    ...
    if (!detailQuery.data)
        return (
            <div className="page">
                <PageHeader title="Submission unavailable" />
            </div>
        );
    ...
    return (
        <div className="page narrow-page">
            <PageHeader
                ...
                description="Your changes will return the resource to the moderation queue."
            />
            <Card className="form-card">
                <form className="submission-form" onSubmit={submit}>
                    ...
                    <Field label="Resource type" required>
                        ...
                        >
                            {RESOURCE_TYPES.filter(
                                ([value]) =>
                                    (detailQuery.data.resource.storageKind ===
                                        "external") ===
                                    (value === "external_link"),
                            ).map(([value, label]) => (
                    ...
                    {detailQuery.data.resource.storageKind === "external" ? (
                        <Field label="Resource link" required>
                            <Input
                                type="url"
                                value={externalUrl}
                                onChange={(event) =>
