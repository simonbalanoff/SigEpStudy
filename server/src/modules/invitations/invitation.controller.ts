...
function activeInvitationFilter(tokenHash: string): Record<string, unknown> {
    const now = new Date();
...
export const resolveInvitation: RequestHandler = async (request, response) => {
...
export const listInvitations: RequestHandler = async (_request, response) => {
...
export const createInvitation: RequestHandler = async (request, response) => {
    ...
    response.status(201).json({
        invitation: {
            ...
            inviteUrl: `${env.APP_URL.replace(/\/$/, "")}/register/${token}`,
        },
    });
};

export const deactivateInvitation: RequestHandler = async (
    request,
    response,
) => {
    const user = request.auth?.user;
    ...
    const invitationId = requireObjectId(
        routeParam(request.params.invitationId, "invitationId"),
        "invitationId",
    );
...
export const listUsers: RequestHandler = async (request, response) => {
    ...
    const [users, total] = await Promise.all([
        User.find().sort({ lastName: 1, firstName: 1 }).skip(skip).limit(limit),
        User.countDocuments(),
    ]);
