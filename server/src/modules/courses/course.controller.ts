...
export const getCourse: RequestHandler = async (request, response) => {
    const courseId = requireObjectId(
        routeParam(request.params.courseId, "courseId"),
        "courseId",
    );
...
export const listCourseProfessors: RequestHandler = async (
    request,
    response,
) => {
    const courseId = requireObjectId(
...
export const searchProfessors: RequestHandler = async (request, response) => {
    ...
    if (input.q)
        filter.normalizedName = {
            $regex: escapeRegex(normalizeName(input.q)),
            $options: "i",
        };
    if (input.courseId) {
...
export const createCourseProfessor: RequestHandler = async (
    request,
    response,
) => {
    ...
    const association = await CourseInstructor.findOneAndUpdate(
        { courseId, professorId: professor._id },
        {
            $set: {
                status:
                    professor.status === "approved" ? "approved" : "pending",
            },
